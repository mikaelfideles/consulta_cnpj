...
def iniciar_coleta():
    output_text.delete(1.0, tk.END)  # Limpa qualquer texto existente no widget de texto
    output_text.insert(tk.END, 'CONSULTA CNPJ SIMPLES NACIONAL\n\n')
    ...
    for i, linha in consulta.iterrows():
        ...
        output_text.insert(tk.END, 'CNPJ: ' + linha['NUM_CNPJ'] + '\n')
        output_text.update()
        ...
        output_text.insert(tk.END, linha['COLETA'] + '\n')
        output_text.update()
...
root = tk.Tk()
root.title("Consulta CNPJ - Simples Nacional")

# Botão para iniciar a coleta
btn_iniciar_coleta = tk.Button(root, text="Iniciar Coleta", command=iniciar_coleta)
btn_iniciar_coleta.pack(pady=20)

# Widget de texto para exibir a saída das consultas
